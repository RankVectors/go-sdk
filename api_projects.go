...
type ApiSyncProjectRequest struct {
	ctx context.Context
	ApiService *ProjectsAPIService
	projectId string
...
func (r ApiSyncProjectRequest) SyncProjectRequest(syncProjectRequest SyncProjectRequest) ApiSyncProjectRequest {
	r.syncProjectRequest = &syncProjectRequest
	return r
}

func (r ApiSyncProjectRequest) Execute() (*SyncProject200Response, *http.Response, error) {
	return r.ApiService.SyncProjectExecute(r)
}
...
Trigger a full sync of project content (used by WordPress and other integrations)
 ...
 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param projectId Unique identifier for the project
 @return ApiSyncProjectRequest
*/
func (a *ProjectsAPIService) SyncProject(ctx context.Context, projectId string) ApiSyncProjectRequest {
	return ApiSyncProjectRequest{
		ApiService: a,
		ctx: ctx,
		projectId: projectId,
...
func (a *ProjectsAPIService) SyncProjectExecute(r ApiSyncProjectRequest) (*SyncProject200Response, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *SyncProject200Response
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ProjectsAPIService.SyncProject")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/projects/{projectId}/sync"
	localVarPath = strings.Replace(localVarPath, "{"+"projectId"+"}", url.PathEscape(parameterValueToString(r.projectId, "projectId")), -1)
	...
	// body params
	localVarPostBody = r.syncProjectRequest
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["ApiKeyAuth"]; ok {
